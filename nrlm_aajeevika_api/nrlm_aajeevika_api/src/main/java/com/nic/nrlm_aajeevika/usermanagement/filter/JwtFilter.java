    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (jwt != null) {
            ...
            if (tokenType.equals(MyConstant.VALIDATED_JWT) || tokenType.equals(MyConstant.EXPIRED_JWT_EXCEPTION)) {
                ...
                ExceptionResponse ex = new ExceptionResponse(HttpStatus.UNAUTHORIZED.value(), new Date(), request.getRequestURI(), message, HttpStatus.UNAUTHORIZED.name());
        ...
        if (loginId != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            if (request.getRequestURI().equals(MyConstant.REFRESH_URL)) {
                ...
                authenticatedUser = (AuthenticatedUser) userDetailsService.validateUser(loginId); // use a different method

                if (loginId.equals(authenticatedUser.getUsername()) && jwt.equals(authenticatedUser.getUserToken())) {
                    //String dbTokenType =  jwtUtil.validateApiTokenType(authenticatedUser.getUserToken());
